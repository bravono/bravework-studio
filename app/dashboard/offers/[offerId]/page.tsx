...
export default function CustomerOfferDetailsPage() {
  ...
  const performOfferAction = useCallback(async (action: 'accept' | 'reject', reason?: string) => {
    if (!offer || offer.status !== 'Pending') {
      alert(`This offer is already ${offer?.status?.toLowerCase() || 'not available'}.`);
      return;
    }
    ...
    try {
      const res = await fetch(`/api/user/custom-offers/${offerId}/${action}`, {
        method: 'POST',
      ...
      setOffer(prev => prev ? { ...prev, status: result.newStatus, rejectionReason: reason } : null); // Update local state
    ...
    <div className="dashboard-page">
      <div className="dashboard-container">
        <div className="dashboard-header">
          ...
          <Link href="/dashboard/offers" className="profile-link">
            <span>&larr; Back to All Offers</span>
          </Link>
        </div>
        ...
        <div className="dashboard-grid">
          <div className="dashboard-card offer-details">
            ...
            <p><strong>Offer Amount:</strong> ${offer.offerAmount.toLocaleString()}</p>
            <p><strong>Description:</strong> {offer.description}</p>
            <p><strong>Created At:</strong> {format(new Date(offer.createdAt), 'MMM dd, yyyy HH:mm')}</p>
            ...
            <p><strong>Service:</strong> {offer.orderService || 'N/A'}</p>
            <p><strong>Order Description:</strong> {offer.orderDescription || 'N/A'}</p>
            <p><strong>Original Budget:</strong> {offer.orderBudget ? `$${offer.orderBudget.toLocaleString()}` : 'N/A'}</p>
            ...
            {!canAct && offer.status !== 'Expired' && (
              <p className="mt-4 text-center text-gray-600">This offer cannot be modified as its status is {offer.status.toLowerCase()}.</p>
            )}
            {isExpired && (
              <p className="mt-4 text-center text-red-600 font-bold">This offer has expired and can no longer be accepted or rejected.</p>
            )}
          </div>
        </div>
      </div>

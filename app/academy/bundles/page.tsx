...
export default function AcademyBundlesPage() {
  ...
  const getDescendants = useCallback((course: Course, allCourses: Course[]): Course[] => {
    let descendants: Course[] = [];
    if (course.childCourseIds && course.childCourseIds.length > 0) {
      const directChildren = allCourses.filter(c => course.childCourseIds?.includes(Number(c.id)));
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
             ...
             <motion.div
              ...
              >
                ...
                <div className="relative z-10 flex-grow text-center md:text-left">
                   ...
                   <h2 className="text-3xl md:text-4xl font-black mb-4 leading-tight">Create a Custom Learning Bundle</h2>
                   <p className="text-blue-100 text-lg mb-8 max-w-xl">
                     Pick any 3 courses and unlock a 20% flat discount + additional savings on gear rentals.
                   </p>
                   <Link
              ...
              >
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-4">Enterprise Solutions</h3>
                <p className="text-gray-500 mb-8 font-medium">
                  Looking for group training or custom curriculum for your organization?
                </p>
                <a
                  href="mailto:hello@braveworkstudio.com?subject=Enterprise Training Inquiry"
                  className="w-full py-4 bg-gray-900 text-white font-black rounded-2xl hover:bg-blue-600 transition-all flex items-center justify-center gap-2"
                >
          ...
          <div className="flex items-center gap-4 mb-12">
            <h4 className="text-sm font-black uppercase tracking-[0.2em] text-gray-400 whitespace-nowrap">Curated Career Paths</h4>
          ...
          {isLoading ? (
            ...
            <div className="py-24 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900">No preset bundles available</h3>
              <p className="text-gray-500">Check back soon for new curated paths or build your own!</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-12">
              {presetBundles.map((bundle, i) => {
                const bundleCourses = bundle.bundleCourses;
                const bundlePrice = bundleCourses.reduce((sum, c) => sum + c.price, 0);
                const discount = bundleCourses.length === 2 ? 0.1 : bundleCourses.length >= 3 ? 0.2 : 0;
                      ...
                      <div className="absolute inset-0 flex items-center justify-center p-8">
                         <div className="relative w-full h-full max-w-lg">
                            ...
                            <div className="absolute inset-0 bg-white rounded-3xl shadow-xl border border-blue-100 flex items-center justify-center overflow-hidden">
                               <img
                                src={bundle.thumbnailUrl || "/assets/Bravework_Studio-Logo-Color.png"}
                                alt={bundle.title}
                                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                              />
                              <div className="absolute inset-0 bg-gradient-to-t from-blue-900/60 to-transparent"></div>
                              <div className="absolute bottom-8 left-8 text-white">
                                 <div className="text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Academy Path</div>
                                 <div className="text-2xl font-black leading-tight">Complete {bundle.title}</div>
                              </div>
                            </div>
                         </div>
                      </div>
                    ...
                    <div className="p-10 flex flex-col flex-grow bg-gradient-to-b from-blue-50/30 to-white">
                      ...
                      <div className="space-y-4 mb-8">
                         {bundleCourses.map((bc, idx) => (
                           <div key={bc.id} className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-blue-50 shadow-sm group/item hover:border-blue-200 transition-colors">
                              ...
                              <div className="flex-grow">
                                ...
                                <span className="text-[10px] text-gray-400 uppercase tracking-widest font-black">{bc.category}</span>
                              </div>
                           </div>
                         ))}
                      </div>
                      ...
                      <div className="mt-auto pt-8 border-t border-blue-50 flex items-center justify-between">
                        <div>
                          <div className="text-sm text-gray-400 line-through font-bold mb-1">
                              {exchangeRates 
                                ? convertCurrency(bundlePrice / KOBO_PER_NAIRA, exchangeRates[selectedCurrency], getCurrencySymbol(selectedCurrency)) 
                                : convertCurrency(bundlePrice / KOBO_PER_NAIRA, 1, "₦")}
                          </div>
                          <div className="text-3xl font-black text-blue-600">
                            {exchangeRates 
                              ? convertCurrency(discountedPrice / KOBO_PER_NAIRA, exchangeRates[selectedCurrency], getCurrencySymbol(selectedCurrency)) 
                              : convertCurrency(discountedPrice / KOBO_PER_NAIRA, 1, "₦")}
                          </div>
                        </div>
                        <Link
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
           <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div>
                 ...
                 <h2 className="text-4xl md:text-5xl font-black mb-8 leading-tight">Empower Your Team with Creative Tech</h2>
                 <p className="text-xl text-gray-400 mb-12 leading-relaxed">
                    Custom-tailored training programs for schools, agencies, and production houses. We help your staff master the latest tools in 3D, Design, and Development.
                 </p>
              ...
              <div className="bg-white/5 backdrop-blur-md rounded-[3rem] p-12 border border-white/10">
                 ...
                 <form className="space-y-4">
                    <input type="text" placeholder="Organization Name" className="w-full px-6 py-4 bg-white/10 border border-white/10 rounded-2xl focus:border-blue-500 outline-none transition-colors" />
                    <input type="email" placeholder="Work Email" className="w-full px-6 py-4 bg-white/10 border border-white/10 rounded-2xl focus:border-blue-500 outline-none transition-colors" />
                    <textarea placeholder="Tell us about your training needs..." rows={4} className="w-full px-6 py-4 bg-white/10 border border-white/10 rounded-2xl focus:border-blue-500 outline-none transition-colors" />

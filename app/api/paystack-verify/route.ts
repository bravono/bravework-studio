    ...
    return await withTransaction(async (client) => {
      ...
      if (referredById) {
        ...
        const paymentsCount = await client.query(
          "SELECT count(*) FROM payments WHERE order_id IN (SELECT order_id FROM orders WHERE user_id = $1)",
          [order.user_id]
        );
        ...
        if (existingEarnings.rows.length === 0) {
          ...
          const amountPaidForCommissionBase =
            actualAmountKobo + walletUsageKobo;
          commissionAmount = Math.round(amountPaidForCommissionBase * 0.1);

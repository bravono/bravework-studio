...
async function loadOrderStatusMap() {
  ...
  orderStatusLoadPromise = (async () => {
    try {
      ...
      const { rows } = await queryDatabase('SELECT order_status_id, name FROM order_statuses');
      ...
      console.error('Failed to load order statuses from DB on startup/request:', error);
...
export async function POST(req: NextRequest) {
  ...
  switch (event.event) {
    case 'charge.success':
      ...
      if (!orderId) {
        console.error(`Webhook for reference ${paystackReference} missing order_id in metadata.`);
        return NextResponse.json({ message: 'Missing order_id in metadata.' }, { status: 400 });
      }
      ...
      try {
        ...
        const { rows: existingPaymentRows } = await queryDatabase(
          'SELECT id FROM payments WHERE paystack_reference = $1 AND paystack_status = $2',
          [paystackReference, 'success'] // Still checking Paystack's own status string here
        );

        if (existingPaymentRows.length > 0) {
          console.log(`Payment with Paystack reference ${paystackReference} already processed. Skipping duplicate webhook.`);
        ...
        if (orderRows.length === 0) {
          console.error(`Order with ID ${orderId} not found in DB. Cannot process webhook.`);
        ...
        const firstPaymentMinimumKobo = Math.ceil(totalExpectedAmountKobo * 0.5); // 50% of total
        ...
        if (!paymentIsFraudulent && (amountPaidToDateKobo + paystackAmountKobo) > totalExpectedAmountKobo) {
            console.error(
                `FRAUD ALERT! Order ID: ${orderId}. Overpayment detected. ` +
                `Total Expected: ${totalExpectedAmountKobo} ${expectedCurrency}, Current Payment: ${paystackAmountKobo}, New Total Paid: ${amountPaidToDateKobo + paystackAmountKobo}`
            );
            paymentIsFraudulent = true;
        ...
        console.log(`Payment record for Paystack Ref ${paystackReference} inserted into 'payments' table.`);

        // --- Update 'orders' table's running total and status (only if payment is not fraudulent) ---
        if (!paymentIsFraudulent) {
          const newAmountPaidToDateKobo = amountPaidToDateKobo + paystackAmountKobo;
          ...
          console.log(`Order ${order.id} updated: New amount_paid_to_date_kobo = ${newAmountPaidToDateKobo}, Status ID = ${newOrderStatusId}`);
          ...
          if (newOrderStatusId === orderStatusMap['paid']) {
              ...
              console.log(`TODO: Grant access to service for ${customer?.email || customerEmail} (Order ID: ${order.id})`);
          } else if (newOrderStatusId === orderStatusMap['partially_paid']) {
              console.log(`TODO: Order ${order.id} is partially paid. Consider conditional access or waiting for full payment.`);
          }
          ...
          const finalOrderStatusName = Object.keys(orderStatusMap).find(key => orderStatusMap[key] === newOrderStatusId) || 'unknown';
          ...
          console.log(`TODO: Send confirmation email to ${customer?.email || customerEmail} for Order ID: ${order.id} (Status: ${finalOrderStatusName})`);
        } else {
            // If payment is fraudulent, you might send a different type of alert email or take other actions
            const finalFraudStatusName = Object.keys(orderStatusMap).find(key => orderStatusMap[key] === newOrderStatusId) || 'unknown_fraud';
            console.warn(`Payment ${paystackReference} for Order ID ${orderId} marked as fraudulent: ${fraudReason} (Status: ${finalFraudStatusName}). No order update or service granted.`);
            ...
            // If a specific fraud status was determined and stored, update the order status as well
             if (newOrderStatusId) {
                ...
                console.log(`Order ${order.id} status updated to fraud-related status: ${newOrderStatusId}`);
             }
        }

      } catch (error) {
        console.error(`Error processing charge.success webhook for Order ID ${orderId}, Ref ${paystackReference}:`, error);
    ...
    case 'charge.failed':
      ...
      try {
        ...
        console.log(`Failed payment record for Paystack Ref ${failedPaystackReference} inserted into 'payments' table.`);

        // Optionally update the order status if this was the first/only attempt and no payments were made yet.
        if (failedOrderId) {
            const { rows: orderRows } = await queryDatabase('SELECT amount_paid_to_date_kobo, status_id FROM orders WHERE id = $1', [failedOrderId]);
            if (orderRows.length > 0 && orderRows[0].amount_paid_to_date_kobo === 0) {
                const failedStatusId = orderStatusMap['failed'];
                await queryDatabase(
                    `UPDATE orders SET status_id = $1, updated_at = NOW() WHERE id = $2`,
                    [failedStatusId, failedOrderId]
                );
                console.log(`Order ${failedOrderId} status updated to 'failed' as no payments were made yet.`);
            }
        }
      } catch (error) {
        console.error(`Error processing charge.failed webhook for Ref ${failedPaystackReference}:`, error);
      }
      break;

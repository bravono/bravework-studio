...
const CourseDetailCard = ({ course, selectedCurrency, exchangeRates }) => {
  ...
  const areAllClassesPassed = (sessions) => {
    ...
    const latestTimestamp = Math.max(...sessions.map((s) => s.timestamp));
  ...
  const isCertificationEnabled = useMemo(
    () =>
      statusLabel === "Completed" || areAllClassesPassed(course.sessionGroup),
    [statusLabel, course.sessionGroup]
  );
  const currentPrice = useMemo(() => {
    const isDiscountActive =
      course.discount &&
      Date.now() > course.discountStartDate &&
      Date.now() < course.discountEndDate;
              ...
              <div className="mb-6">
                ...
                <ul className="space-y-3 pl-0">
                  {course.sessionGroup.map((session, index) => {
                    ...
                    const isNext = nextSession && nextSession.id === session.id;
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          >
                            ...
                            {format(sessionDate, "hh:mm a")} ({session.duration}{" "}
                            hr{session.duration > 1 ? "s" : ""})
                          </p>
                        </div>

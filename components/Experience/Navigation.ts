...
export default class Navigation {
  ...
  setView() {
    ...
    /**
     * Window mouse movement (for hover sway)
     */
    this.view.onMouseMoveWindow = (_event) => {
      this.view.mouse.x = (_event.clientX / window.innerWidth) * 2 - 1;
      this.view.mouse.y = -(_event.clientY / window.innerHeight) * 2 + 1;
    };
    window.addEventListener("mousemove", this.view.onMouseMoveWindow);
    ...
    this.view.onTouchStart = (_event) => {
      if (
        _event.target === this.targetElement ||
        this.targetElement.contains(_event.target)
      ) {
        _event.preventDefault();
      }
    ...
    this.targetElement.addEventListener("contextmenu", this.view.onContextMenu);

    /**
     * Wheel
     */
    this.view.onWheel = (_event) => {
      // Only zoom and preventDefault if user holds Shift
      if (_event.shiftKey) {
        _event.preventDefault();
  ...
  destroy() {
    window.removeEventListener("mousemove", this.view.onMouseMoveWindow);
  ...
  update() {
    // Adjust sensitivity and speed based on whether the user is actively dragging
    this.view.drag.sensitivity = this.view.drag.active ? 2.5 : 1.0;
    ...
    // Apply gentle mouse sway when the user is not actively dragging
    let thetaOffset = 0;
    let phiOffset = 0;
    if (!this.view.drag.active) {
      thetaOffset = this.view.mouse.x * 0.15;
      phiOffset = this.view.mouse.y * 0.08;
    }

    const targetSpherical = this.view.spherical.smoothed.clone();
    targetSpherical.theta += thetaOffset;
    targetSpherical.phi += phiOffset;

    const viewPosition = new THREE.Vector3();
    viewPosition.setFromSpherical(targetSpherical);

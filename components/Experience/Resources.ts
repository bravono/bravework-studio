...
export default class Resources extends EventEmitter
{
    constructor(_assets)
    {
        ...
        this.loader.on('fileEnd', (_resource, _data) =>
        {
            ...
            if (this.experience.onProgress)
            {
                const total = this.groups.current.toLoad
                const loaded = this.groups.current.loaded
                this.experience.onProgress(loaded / total)
            }
        })
        ...
        this.loader.on('end', () =>
        {
            ...
            {
                this.trigger('end')
                if (this.experience.onReady)
                {
                    this.experience.onReady()
                }
            }
        })
    }

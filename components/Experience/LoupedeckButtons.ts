...
export default class LoupedeckButtons
{
    ...
    setModel()
    {
        ...
        // Children
        const children = this.resources.items.loupedeckButtonsModel.scene.children.map(c => c.clone())
        children.sort((_a, _b) =>
        {
            if(_a.name < _b.name)
                return -1
    ...
    setAnimation()
    {
        ...
        this.animation.play()
        this.animation.intervalId = window.setInterval(this.animation.play, 5000)
    }
    ...
    destroy()
    {
        if (this.animation && this.animation.intervalId)
        {
            window.clearInterval(this.animation.intervalId)
        }

...
import vertexShader from './shaders/baked/vertex'
import fragmentShader from './shaders/baked/fragment'
...
export default class Baked
{
    ...
    setModel()
    {
        ...
        const originalMesh = this.resources.items.roomModel.scene.children[0]
        this.model.mesh = originalMesh ? originalMesh.clone() : null

        if (!this.model.mesh)
        {
            console.error('roomModel mesh not found')
            return
        }
